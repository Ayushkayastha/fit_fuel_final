...
import '../data/lunch_provider.dart';
...
class _LunchState extends ConsumerState<Lunch> {
  ...
  void initState() {
    super.initState();
    Future.microtask(() => ref.refresh(lunchProvider));
  ...
  Widget build(BuildContext context) {
    ...
    final lunchAsync = ref.watch(lunchProvider);
    return Scaffold(
      appBar: AppBar(
        leading: IconButton(
            ...
            Expanded(
              child: Center(
                child: lunchAsync.when(
                  data: (entries) {
                    if (entries.isEmpty) return const Center(child: Text("No lunch items"));
